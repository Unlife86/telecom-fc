 ...
 */
(function(){
	var J,H,I=Math.abs,ac=Math.sin,v=Math.cos,q=Math.max,aw=Math.min,aj=Math.ceil,C=Math.sqrt,al=Math.pow,f={},h={},k={0:"0,",1:"17,",2:"34,",3:"51,",4:"68,",5:"85,",6:"102,",7:"119,",8:"136,",9:"153,",a:"170,",A:"170,",b:"187,",B:"187,",c:"204,",C:"204,",d:"221,",D:"221,",e:"238,",E:"238,",f:"255,",F:"255,"},w,b,N,ay,E,az,B=document,n,a={};for(J=0;J<256;++J){H=J.toString(16);
	if(J<16){
		H="0"+H}h[H]=h[H.toUpperCase()]=J.toString()+","
	}
	function ad(i){
	...
	E.cross=function(j){
		var i=this.y*j.z-this.z*j.y,aB=this.z*j.x-this.x*j.z,aA=this.x*j.y-this.y*j.x;
		return new Z(i,aB,aA)
	};
	E.angle=function(j){
		var i=this.dot(j),aA;
		if(i==0){
			return Math.PI/2}aA=i/(this.length()*j.length());
			if(aA>=1){return 0}
			if(aA<=-1){return Math.PI}
			return Math.acos(aA)
	};
	E.unit=function(){
		var i=this.length();
		return new Z(this.x/i,this.y/i,this.z/i)
	};
	function ae(aA,j){
		j=j*Math.PI/180;
		aA=aA*Math.PI/180;
		var i=ac(aA)*v(j),aC=-ac(j),aB=-v(aA)*v(j);
		return new Z(i,aC,aB)
	}
	function O(i){
